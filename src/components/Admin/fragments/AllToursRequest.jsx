...
class AllToursRequest extends Component {
    constructor(props) {
        super(props);
        this.state = {
        render: '',
  ...
  approvedHandler(id){
    console.log(id)
   Axios.put(`http://localhost:4000/api/tours/approved/${id}`,{Status:true})
   .then(res =>{
    alert("Approved Tour")
   })
   .catch(err=>{
 ...
 unapprovedHandler(id){
  Axios.delete(`http://localhost:4000/api/tours/unapproved/${id}`)
  .then(res =>{
   alert("Tour Delete")
  })
  .catch(err=>{
    ...
    render() {
        return (
            <div style={{marginLeft:"250px", marginTop:"120px"}}>
                <h1>
                  Welcome to  All Tour's Request !
                </h1>
                {this.state.Tours.length == 0 ? (
          <p>There are no Tour's Request</p>
        ) : (
      ...
      <Table className={classes.table} aria-label="customized table">
        <TableHead>
          <TableRow>
            <StyledTableCell>Tour's Request Title</StyledTableCell>
            ...
            <StyledTableRow key={Tour._id}>
              ...
              <Button
                  onClick={()=> this.approvedHandler(Tour._id)}
                >
                  Approved
                </Button>     

                <Button
                  onClick={()=> this.unapprovedHandler(Tour._id)}
                >
                  UnApproved
                </Button>  
...
export default withRouter(AllToursRequest);